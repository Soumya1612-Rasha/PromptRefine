...
class XqorConfig(datasets.BuilderConfig):
...
class XQOR(datasets.GeneratorBasedBuilder):
    """The XQOR dataset"""
    BUILDER_CONFIGS = [
        XqorConfig(
            name="xqor",
            version=datasets.Version("1.0.0"),
            description="XQOR dataset with custom language codes",
    ...
    def _split_generators(self, dl_manager):
        ...
        if all_data:
            ...
            final_path_name_prefix = f"xorqa_{lang_code}"
            ...
            return [
                datasets.SplitGenerator(
                    ...
                    gen_kwargs={"filepath": downloaded_files + f"/xorqa_in/{final_path_name_prefix}"}
                ),
                datasets.SplitGenerator(
                    name=datasets.Split.VALIDATION,
                    gen_kwargs={"filepath": downloaded_files + f"/xorqa_in/xorqa_{test_lang_code}_dev.json"}
                ),
                datasets.SplitGenerator(
                    name=datasets.Split.TEST,
                    gen_kwargs={"filepath": downloaded_files + f"/xorqa_in/xorqa_{test_lang_code}_test.json"}
                )
            ]
            ...
            return [
                datasets.SplitGenerator(
                    name=datasets.Split.TRAIN,
                    gen_kwargs={"filepath": downloaded_files + f"/xorqa_in/xorqa_{lang_code}_train.json"}
                ),
                datasets.SplitGenerator(
                    name=datasets.Split.VALIDATION,
                    gen_kwargs={"filepath": downloaded_files + f"/xorqa_in/xorqa_{test_lang_code}_dev.json"}
