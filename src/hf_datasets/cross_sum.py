...
class CrossSum(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        CrossSumConfig(
            ...
            description="Cross Sum dataset with custom language codes",
    ...
    def _split_generators(self, dl_manager):
        ...
        if all_data:
            ...
            final_path_name_prefix = f"crosssum_english-{lang_code}-"
            for code in high_lang_code:
                    final_path_name_prefix = final_path_name_prefix + f"{code}"
            
            final_path_name_prefix = final_path_name_prefix + "_dev.json"
            
            print(final_path_name_prefix)
            return [
                datasets.SplitGenerator(
                    name=datasets.Split.TRAIN,
                    gen_kwargs={"filepath": downloaded_files + f"/crosssum_in/{final_path_name_prefix}"}
                ),
                datasets.SplitGenerator(
                    name=datasets.Split.VALIDATION,
                    gen_kwargs={"filepath": downloaded_files + f"/crosssum_in/crosssum_english-{test_lang_code}_dev.json"}
                ),
                datasets.SplitGenerator(
                    name=datasets.Split.TEST,
                    gen_kwargs={"filepath": downloaded_files + f"/crosssum_in/crosssum_english-{test_lang_code}_test.json"}
                )
            ]
            ...
            return [
                datasets.SplitGenerator(
                    name=datasets.Split.TRAIN,
                    gen_kwargs={"filepath": downloaded_files + f"/crosssum_in/crosssum_english-{lang_code}_train.json"}
                ),
                datasets.SplitGenerator(
                    name=datasets.Split.VALIDATION,
                    gen_kwargs={"filepath": downloaded_files + f"/crosssum_in/crosssum_english-{test_lang_code}_dev.json"}
