...
class BaseDatasetReader(torch.utils.data.Dataset):
    ...
    def __init__(self, task_name, model_name, field, dataset_path=None, dataset_split=None, ds_size=None, lang_name=None, high_lang_name=None, all_data=None, test_lang_name=None) -> None:
        
        self.tokenizer = get_tokenizer(model_name)
        self.init_dataset(task_name, field, dataset_path, dataset_split, ds_size, lang_name=lang_name, high_lang_name = high_lang_name, all_data = all_data, test_lang_name = test_lang_name)
        

    def init_dataset(self, task_name, field, dataset_path, dataset_split, ds_size, truncation=True, lang_name=None, high_lang_name=None, all_data=None, test_lang_name=None):
        # pdb.set_trace()
        self.dataset_wrapper = get_dataset_wrapper(task_name,
                                                   dataset_path=dataset_path,
                                                   dataset_split=dataset_split,
                                                   ds_size=ds_size,
                                                   lang_name=lang_name, high_lang_name=high_lang_name, all_data=all_data, test_lang_name=test_lang_name )
