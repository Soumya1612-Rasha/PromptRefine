...
class ABC:
    ...
    def __init__(self, dataset_path=None, dataset_split=None, ds_size=None, lang_name=None, high_lang_name=None, all_data=None,test_lang_name=None ):
        if dataset_path is None or not os.path.exists(dataset_path):
            print("here")
            # pdb.set_trace()
            # print(f"{lang_code}")
            self.dataset = load_dataset(self.hf_dataset, self.hf_dataset_name, lang_name=lang_name, test_lang_name=test_lang_name,
    high_lang_name=high_lang_name, all_data = all_data, trust_remote_code=True)
        else:
